...
const ForgotForm = () => {
  ...
  const [errMsg, setErrMsg] = useState("รีเซ็ตรหัสผ่านบัญชีไม่สำเร็จ");

  const handleSignup = async (e: any) => {
    e.preventDefault();
    try {
      await resetPassword(data.email);
      setErrMsg("ระบบได้ทำการรีเซ็ตรหัสของท่าน โปรดตรวจสอบ Email ของท่าน");
      errRef?.current.open();
    } catch (error: FirebaseError | any) {
      if (error.message.includes("invalid-email")) {
        setErrMsg("บัญชีไม่ถูกต้อง");
      } else {
        setErrMsg("รีเซ็ตรหัสผ่านบัญชีไม่สำเร็จ");
      }
      errRef?.current.open();
    }
  };
  ...
  const isErrr = errMsg.includes("ไม่");
      ...
      <div className="flex flex-col items-center justify-center px-3 py-8 mx-auto h-screen lg:py-0">
        <div className="w-full max-w-sm rounded-lg border border-gray-200 bg-white p-4 py-8 shadow-md dark:border-gray-700 dark:bg-gray-800 sm:p-6 sm:py-10 md:p-8 md:py-14">
          <Alert
            ref={errRef}
            timer={2500}
            headline={isErrr ? "Error" : "สำเร็จ"}
            headlineColor={isErrr ? "text-red-600" : "text-green-600"}
            bgColor={isErrr ? "bg-red-100" : "bg-green-100"}
          ...
          <form action="" className="group" onSubmit={handleSignup}>
            <div className="mb-3">
              ...
              >
                อีเมล <span className="text-gray-400">(ที่ลงทะเบียน)</span>
              </label>
              <input
                type="email"
                name="email"
